'''
#
# Notes
#


Guy that has the exact setup I want to create:
> Homemade 32-bit CPU and OS: https://www.youtube.com/watch?v=OQKCLf32TB8&t=83s
> See Homemade 32-bit Docs

Open Sparc
> This is a 64 bit threading CPU. Would be good to base stuff around it
> > https://github.com/freecores/sparc64soc
> > https://www.oracle.com/servers/technologies/opensparc-t2-page.html


OS Creation:
> https://wiki.osdev.org/Introduction
> https://www.youtube.com/watch?v=mpPbKEeWIHU&list=PLxN4E629pPnJxCQCLy7E0SQY_zuumOVyZ
> https://www.youtube.com/watch?v=MwPjvJ9ulSc&t=9s


Add Verlog
> https://github.com/Obijuan/open-fpga-verilog-tutorial/wiki/Home_EN
> http://digitaljs.tilk.eu/
> https://hdlbits.01xz.net/wiki/Problem_sets#Getting_Started


'''